...
public:
   ...
   * code when object is instatiated in .bss section \warning If object is
   * instantiated on stack, heap or inside noinit section then the contents have
   * to be explicitly cleared before use \param dummy Ignored
   */
   ...
   * to be inserted into buffer \return True if data was inserted and callback
   * called
   */
  ...
  bool remove() {
    ...
    if (tmp_tail == head.load(std::memory_order_relaxed))
      ...
      tail.store(
          ++tmp_tail,
          index_release_barrier); // release in case data was loaded/used before
